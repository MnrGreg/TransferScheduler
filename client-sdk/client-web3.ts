...
async function main() {
  ...
  gasTokenAddress = await transferContract.methods.getGasToken().call();
  let gasCommissionPercentage: bigint;
  gasCommissionPercentage = await transferContract.methods.getGasCommissionPercentage().call();
  ...
  const fillerGasTokenContract = new web3.eth.Contract(gasTokenABI, gasTokenAddress);
  ...
  const nonce = 45873456726;
  const relayCharge = maxBaseFee * 100000 * (1 + Number(gasCommissionPercentage) / 100);
  ...
  try {
    txid = await transferContract.methods.queueScheduledTransfer(
      ScheduledTransfer.owner,
      ...
      BigInt(ScheduledTransfer.maxBaseFee),
      signature).send({
        from: accounts[3]
      });
  } catch (error) {
  ...
  console.log(`Broadcasted queueScheduledTransfer transaction: ${txid.transactionHash}`);

  // try {
  //   txid = await transferContract.methods.executeScheduledTransfer(ScheduledTransfer, signature).send({
  //     from: accounts[3]
  //   });
  // } catch (error) {
  //   throw (error);
